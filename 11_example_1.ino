...
#define _DUTY_NEU 1474 // servo neutral position (90 degree)
#define _DUTY_MAX 2401 // servo full counterclockwise position (180 degree)

// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, alpha, degree; // unit: mm
...
// get a distance reading from the USS
  if (dist_min <= USS_measure(PIN_TRIG,PIN_ECHO) && USS_measure(PIN_TRIG,PIN_ECHO) <= dist_max) {
...
// adjust servo position according to the USS read value
  if(dist_ema <= 180) {
    myservo.writeMicroseconds(_DUTY_MIN); // 0 degree
  }
  else if(180 < dist_ema && dist_ema < 360){
    degree = 10.3 * dist_ema - 1307;
     myservo.writeMicroseconds(degree);
  }
  else {
    myservo.writeMicroseconds(_DUTY_MAX); // 180 degree
  }
